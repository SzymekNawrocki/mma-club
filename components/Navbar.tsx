...
const NavLink = ({ href, active, children, onClick }: NavLinkProps) => (
  <Link
    className={`md:flex px-4 text-whiteText rounded-full bg-emerald300 hover:bg-emerald500 ${active ? 'font-bold' : 'font-normal'} text-lg py-2`}
    onClick={onClick}
    href={href}
  >
    {children}
...
const Navbar = () => {
  ...
  return (
    <nav className={`py-2 w-full fixed top-0 z-50 transition-colors duration-300 bg-neutral50 ${scrolled ? 'shadow-lg' : ''}`}>
      <div className="max-w-4xl mx-auto flex justify-between items-center px-4">
        <Link className="bg-emerald300 text-whiteText rounded-full py-3 px-3 flex font-bold text-lg gap-2" href="/">
          Just Fight <FaFistRaised />
        </Link>
